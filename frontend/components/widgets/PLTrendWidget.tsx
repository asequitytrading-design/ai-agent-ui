...
interface PLTrendStale {
  ticker: string;
  last_valid_close_date: string;
  days_stale: number;
}

function staleItemsForPL(stale: PLTrendStale[]): StaleChipItem[] {
  return stale.map((s) => ({
    key: s.ticker,
    primary: s.ticker,
    secondary: `${s.last_valid_close_date} · ${s.days_stale}d old`,
  }));
}
    ...
    <div className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
      <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <StaleTickerChip
            items={staleItemsForPL(stale)}
            summaryLabel={
              stale.length === 1
                ? "holding using previous close"
                : "holdings using previous close"
            }
            tooltipTitle="Showing yesterday's close for:"
            tooltipFooter="Upstream data hasn't settled. Auto-clears on next refresh."
            testId="pltrend-stale-chip"
          />
        </div>
