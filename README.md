...
- **Advanced Analytics** *(Sprint 9)* — pro/superuser route `/advanced-analytics` with 7 NSE-bhavcopy reports + an in-app **Help** tab documenting all 56 columns. Reports: Current Day Upmove, Previous Day Breakout, MoM/WoW Volume + Delivery, Two/Three-Day Scan, Top 50 by Delivery Qty. Default filters India + Stocks-only; market / ticker_type / debounced ticker search. All reports anchor to `MAX(date) FROM nse_delivery` so OHLCV + delivery describe the same trading day across weekends/holidays. Two-layer Redis cache delivers ~1500× warm-path speedup on filter/sort. Backed by 4 new Iceberg tables: `nse_delivery` (NSE bhavcopy), `fundamentals_snapshot` (3y/5y CAGR + ROCE), `corporate_events`, `promoter_holdings` (BSE). Daily India pipeline now 9 steps; `iceberg_maintenance` runs last so it backs up + compacts the AA tables. SOP: [docs/backend/advanced-analytics-rollout.md](docs/backend/advanced-analytics-rollout.md). Architecture pattern: `aa-as-of-anchor-and-two-layer-cache` Serena memory.
- **ScreenQL extensions** *(Sprint 9 polish)* — query language now exposes **55 fields** (was 30) with the new bhavcopy / AA mirror added. New `LIKE` operator for case-insensitive substring search on text fields (`ticker LIKE "RELIA"`). New **Tables sub-mode** lets pro/superuser pick from a whitelist of 7 Iceberg tables (`nse_delivery`, `fundamentals_snapshot`, `corporate_events`, `promoter_holdings`, `ohlcv`, `dividends`, `quarterly_results`) and query that table's columns directly with the same DSL grammar — hard `LIMIT ≤ 1000`, ticker-scope filter for general users.
...
- **Recommendation Performance analytics** — cohort-bucketed view at `/analytics/analysis?tab=recommendations&subtab=performance`. Granularity drives the primary horizon (Weekly→7d, Monthly→30d, Quarterly→90d). 14-month hard-cap retention via daily `recommendation_cleanup` job. Self-healing outcomes pipeline at 4 horizons {7, 30, 60, 90}: any rec ≥ N days old without an outcome at horizon N is picked up next run, even if the daily run was skipped. Activity chart + KPI tiles with What / How / Formula info-tooltips. See [docs/backend/recommendation-performance.md](docs/backend/recommendation-performance.md).
...
- **RSC dashboard** — `/dashboard` is a React Server Component (Next.js 16) that pre-fetches `/v1/dashboard/home` server-side using an HttpOnly access-token cookie set on `/v1/auth/login`. Streamed HTML carries real data; SWR's `fallbackData` seeds the client tree on hydrate, so first render paints with no skeleton step. Edge `proxy.ts` gates protected routes on cookie presence. Pattern documented in [docs/frontend/ssr-patterns.md](docs/frontend/ssr-patterns.md).
- **Containerized Lighthouse audit** — `Dockerfile.perf` + `frontend-perf` compose service runs Playwright-authenticated Lighthouse against 9 base + 25 tab routes (`docker compose --profile perf run --rm perf`). Aggregate JSON at `.lighthouseci/pw-lh-summary.json`. Used as the perf-regression gate for every Sprint 8+ frontend story. Full numbers in [docs/frontend/bundle-analysis.md](docs/frontend/bundle-analysis.md).
