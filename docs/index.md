...
- **Advanced Analytics** *(Sprint 9)* — pro/superuser route `/advanced-analytics` with 7 NSE-bhavcopy reports (Current Day Upmove, Previous Day Breakout, MoM/WoW Volume+Delivery, Two/Three-Day Scan, Top 50 by Delivery Qty) plus an in-app Help tab documenting all 56 columns. Default filters India + Stocks-only; market / ticker_type / debounced ticker search; all reports anchor to `MAX(date) FROM nse_delivery` so volume + delivery describe the same trading day across weekends and holidays. Two-layer Redis cache delivers ~1500× warm-path speedup. Backed by 4 new Iceberg tables: `nse_delivery`, `fundamentals_snapshot`, `corporate_events`, `promoter_holdings`. Pipeline doc: [backend/advanced-analytics](backend/advanced-analytics.md); cutover SOP: [backend/advanced-analytics-rollout](backend/advanced-analytics-rollout.md).
- **ScreenQL universal screener** — text-based stock query language with **55 queryable fields** across 10 Iceberg tables (Sprint 9 added bhavcopy + AA mirror). Operators `=`, `!=`, `>`, `<`, `>=`, `<=`, `LIKE` (case-insensitive substring), `CONTAINS` (tags). Plus a **Tables sub-mode** that lets pro/superuser pick a single Iceberg table from a whitelist and query its columns directly with the same DSL grammar — read-only, mandatory `LIMIT ≤ 1000`, ticker-scope filter for general users.
